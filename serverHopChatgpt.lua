...
DragFrame.Size = UDim2.new(0, 220, 0, 280)
DragFrame.Position = UDim2.new(0.7, 0, 0.1, 0)
...
TitleLabel.Text = "Servidores con Objetivo"
TitleLabel.Parent = DragFrame

local StopButton = Instance.new("TextButton")
StopButton.Size = UDim2.new(1, 0, 0, 30)
StopButton.Position = UDim2.new(0, 0, 1, -30)
...
StopButton.MouseButton1Click:Connect(function()
    _G.running = false
    StopButton.Text = "BUSQUEDA DETENIDA"
    StopButton.BackgroundColor3 = Color3.fromRGB(150, 150, 150)
end)

local function addServerToList(target, jobId)
    for _, entry in ipairs(foundServers) do
        if entry.jobId == jobId then return end
    end
...
-- Función simulada de huntingLoop
local function huntingLoop()
    while _G.running do
        local servers = {"1", "2", "3"} -- ejemplo, reemplaza con getActiveServers()
        for _, serverId in ipairs(servers) do
            if not _G.running then break end
            local targets = {
                {name = "Tralalero Tralala", earnings = 999999999, distance = 20, position = Vector3.new()}
            }
            for _, target in ipairs(targets) do
                addServerToList(target, serverId)
            end
