local CONFIG = {
    ...
    SCAN_RADIUS = 5000,
    SERVER_HOP_DELAY = 5, -- Aumentado para mayor estabilidad
    MAX_SERVERS = 25,
    DEBUG_MODE = true
}
        ...
        for _, obj in ipairs(location:GetChildren()) do
            if obj:IsA("Model") then
                ...
                if string.find(obj.Name:lower(), player.Name:lower()) or 
                   (obj:GetAttribute("Owner") and obj:GetAttribute("Owner") == player.Name) then
                    -- Buscar el objetivo en esta base
                    for _, descendant in ipairs(obj:GetDescendants()) do
                        -- Por nombre
                        if descendant.Name:lower():find(CONFIG.TARGET_PATTERN:lower()) then
                        ...
                        if (descendant:IsA("TextLabel") or descendant:IsA("TextButton")) and 
                           descendant.Text:lower():find(CONFIG.TARGET_PATTERN:lower()) then
                            return true, player.Name, descendant:GetFullName()
                        end
                        
                        -- Por valor de earnings
                        if descendant:IsA("NumberValue") and descendant.Name:lower():find("earnings") and
                           descendant.Value == CONFIG.TARGET_EARNINGS then
                            return true, player.Name, descendant:GetFullName()
                        end
                    end
                end
...
local function deepScan()
    ...
    local function scanRecursive(parent)
        for _, child in ipairs(parent:GetChildren()) do
            ...
            if child.Name and string.find(child.Name:lower(), CONFIG.TARGET_PATTERN:lower()) then
                match = true
            end
